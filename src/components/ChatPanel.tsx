        ...
        <div className="h-full w-full grid grid-rows-[auto_1fr_auto] min-h-0 bg-linear-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800" style={{ background }}>
                ...
                {agent.messages.map((message) => (
                    <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`flex max-w-xs lg:max-w-md xl:max-w-lg ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                            <div className={`w-8 h-8 rounded-full grid place-items-center text-white font-semibold text-sm shrink-0 ${message.role === 'user' ? 'ml-3 bg-linear-to-br from-green-500 to-emerald-600' : 'mr-3 bg-linear-to-br from-blue-500 to-purple-600'}`}>
                            ...
                            `}>
                                <p className="text-sm leading-relaxed whitespace-pre-wrap"> {message.content instanceof Object && 'text' in message.content ? message.content.text : message.content}</p>
                                <p className={`text-xs mt-1 ${message.role === 'user' ? 'text-green-100' : 'text-slate-500 dark:text-slate-400'}`}>
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm">
                ...
                <div className="flex justify-between items-center mt-2 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    {agent.isRunning && <span>Sending...</span>}
                </div>
            </div>
        </div>
    );
