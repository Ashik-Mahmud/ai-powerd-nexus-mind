...
import { LucideDownload, LucideFileImage, LucideFileSpreadsheet, LucideFileText, LucideShare2, LucideTrash } from 'lucide-react';
...
interface AnimatedListProps {
  items?: string[] | SingleItemType[];
  onItemSelect?: (item: string | SingleItemType, index: number) => void;
...
const FilesAnimatedList: React.FC<AnimatedListProps> = ({
  items = [
    'Item 1',
    'Item 2',
    'Item 3',
  ...
  const handleItemClick = useCallback(
    (item: string | SingleItemType, index: number) => {
      setSelectedIndex(index);
      if (onItemSelect) {
    ...
    <div className={`relative  ${className} `}>
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div className={`p-4 bg-slate-50 shadow border dark:bg-slate-800 w-full rounded-lg ${selectedIndex === index ? 'bg-slate-100 dark:bg-slate-800' : ''} ${itemClassName}`}>
              {
                typeof item === 'string' ? (
                  <p className="text-slate-400 m-0">{item}</p>
                ) : (
                  <div className="text-slate-400 text-sm mt-1 flex items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="info">
                        ...
                        <p className="m-0">Modified: {item.modified}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <LucideDownload className="h-6 w-6" />
...
export default FilesAnimatedList;
